  ...
  const benefits = [
    {
      ...
      description:
        "Write-back tools only create pending proposals. Nothing changes in Odoo until a person clicks Approve.",
      icon: <ShieldIcon className="text-brand size-6" />,
    },
    {
      title: "SQL guardrails",
      description:
        "A single read-only SELECT, table allowlist, statement timeout, and row cap — prompt injection can't write or scan.",
      icon: <ScreenCogIcon className="text-brand size-6" />,
    },
    {
      title: "Schema grounding",
      description:
        "A compact Odoo schema glossary in the system prompt keeps generated SQL on real tables and fields.",
      icon: <ReuseBrainIcon className="text-brand size-6" />,
    },
    {
      title: "Durable memory",
      description:
        "Chat context is stored in Postgres and restored after refresh, so the conversation survives reloads.",
      icon: <RealtimeSyncIcon className="text-brand size-6" />,
    },
    {
      title: "Evaluated, not hoped",
      description:
        "A golden-question suite runs the real agent against a seeded database — 12/12 passing, nightly in CI.",
      icon: <GraphIcon className="text-brand size-6" />,
    },
    {
      title: "Demo-safe by default",
      description:
        "DEMO_MODE disables approvals for public deployments, and a scheduled reset re-seeds Odoo and wipes history.",
    ...
    <Container className="border-divide relative overflow-hidden border-x px-4 py-20 md:px-8">
      <div className="relative flex flex-col items-center">
        ...
        <SectionHeading className="mt-4">
          Built to be trusted with a live ERP
        </SectionHeading>

        <SubHeading as="p" className="mx-auto mt-6 max-w-lg">
          The interesting part isn&apos;t that it answers — it&apos;s that it
          answers safely, and proves it.
        </SubHeading>
      </div>
